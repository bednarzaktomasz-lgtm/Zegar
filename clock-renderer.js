...
class ClockRenderer {
  ...
  updateHands() {
    ...
    this.hourHand.setAttribute('transform', `rotate(${hourDeg}, 200, 200)`);
    
    // Minute hand rotation
    const minuteDeg = (this.time.m / 60) * 360;
    this.minuteHand.setAttribute('transform', `rotate(${minuteDeg}, 200, 200)`);
  }
  ...
  handleStart(evt) {
    ...
    if (dist < radius) {
      ...
      if (distToMinute < threshold && dist > radius * 0.4) {
        ...
        this.isDragging = true;
        this.activeHand = 'minute';
        this.updateTimeFromAngle(mouseAngle);
      }
    }
  }
